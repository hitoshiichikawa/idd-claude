#!/usr/bin/env bash
# =============================================================================
# idd-claude install helper — library functions（#473）
#
# install.sh から `source` される関数定義専用ファイル。トップレベル副作用なし
# （関数定義のみ）。install.sh が SCRIPT_DIR 解決直後に BASH_SOURCE 相対パスで
# source する。配置先: install.sh と同じディレクトリ（リポジトリ root）。
#
# 内容:
#   - 配置ヘルパー（log_action / classify_action / copy_template_file 等）
#   - ラベル自動セットアップ（Issue #85）
#   - 履歴持ち込み（inherited specs / claude branches）検出（Issue #115）
#   - main フロー関数 3 つ（parse_install_args / run_repo_install / run_local_install）
#     install.sh 側の対応する `if` ブロックから呼び出される
#
# 依存: install.sh が本ファイルの source 前に定義するグローバル変数
#   （DRY_RUN / FORCE / FORCE_CLAUDE_MD / SKIP_LABELS / REPO_PATH / INSTALL_LOCAL /
#    INSTALL_REPO / SCRIPT_DIR / REPO_TEMPLATE_DIR / LOCAL_WATCHER_DIR）を参照する。
#
# 単体取得経路について: setup.sh 経由の `curl | bash` は先に repo 全体を
# clone してから install.sh を起動するため（setup.sh 129-134 行目）、
# 本ファイルを curl で単体取得する経路は存在しない（#473 事前判断: 案A）。
# =============================================================================
#
# SC2034 file-wide 抑止の根拠（#473 install-lib.sh 分割の副作用 / watcher module 分割
# （#464 SC2153 / #469 SC2034 等）と同種の cross-file 可視性喪失）:
#   INSTALL_LOCAL / INSTALL_REPO は本ファイルの parse_install_args 内で代入するが、
#   参照は install.sh 側の main フロー（`if $INSTALL_REPO; then` 等）にあるため、
#   単一ファイル静的解析では「未使用」に見える。変数移動対象自体は無改変
#   （#455 共通規約）のため file-wide で抑止する。
# shellcheck disable=SC2034

# ─────────────────────────────────────────────────────────────
# 配置ヘルパー関数群
#   すべて DRY_RUN / FORCE をグローバル変数として参照する。
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# main フロー関数群
#   install.sh 側の該当 `if` ブロックから呼び出される。install.sh 内での元の
#   行位置と処理内容は不変（本体の抽出のみ、ロジックの書き換えなし）。
# ─────────────────────────────────────────────────────────────

# parse_install_args <args...>
#   install.sh の CLI 引数をパースし、REPO_PATH / INSTALL_LOCAL / INSTALL_REPO /
#   DRY_RUN / FORCE / FORCE_CLAUDE_MD / SKIP_LABELS をグローバル変数として設定する。
#   install.sh から `parse_install_args "$@"` で呼び出される（#473）。
parse_install_args() {
while [[ $# -gt 0 ]]; do
  case $1 in
    --repo)
...
# run_repo_install
#   対象リポジトリへのテンプレート配置フロー（CLAUDE.md / agents / rules /
#   issue template / workflow / labels script 配置 → ラベル自動セットアップ →
#   履歴持ち込み検出）。install.sh 側の `if $INSTALL_REPO; then` から呼ばれる（#473）。
run_repo_install() {
...
# run_local_install
#   ローカル PC への watcher 一式配置フロー（bin/*.sh・modules/・guard hook 配置 →
#   OS 別セットアップ案内 → 前提ツールチェック）。install.sh 側の
#   `if $INSTALL_LOCAL; then` から呼ばれる（#473）。
run_local_install() {
  echo ""
