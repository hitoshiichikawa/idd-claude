...
3. **設計 PR は別運用**: 上記 1 段目のスコープ警告のとおり、`claude/issue-<N>-design-*` PR には
   ...
   そのまま適用すると永久 BLOCKED 状態となる。`DESIGN_REVIEWER_ENABLED=true`（#407）で設計 PR 用
   `claude-review` publisher を有効化するか、GitHub branch protection の rule のブランチ pattern
...
5. **停滞 PR 可視化（#412 Req 4）**: `claude-review` を required status に追加した後、
   ...
   - 当該 PR が設計 PR である → 設計 PR 用の `DESIGN_REVIEWER_ENABLED=true` を有効化するか、
