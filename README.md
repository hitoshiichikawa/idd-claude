...
| **PR Iteration out-of-scope 第 3 判定**（PR Iteration / Adjudicator が「正当だが当該 impl PR の権限では是正できない（要 design/spec 変更・spec-stale）」指摘を **`out-of-scope`（第 3 判定）** に分類し、(1) その指摘で iteration round を消費させず、(2) legitimate=0 かつ out-of-scope≥1 のとき `needs-iteration` を外して既定経路（`needs-decisions`）へ人間判断還流、(3) Developer 応答の構造化マーカー `OUT-OF-SCOPE: design` / `OUT-OF-SCOPE: spec-stale` と内容ベース fingerprint no-progress 判定で「設計レベル指摘の堂々巡りによる `max_rounds` 消尽 → `claude-failed`」を防ぐ。gate OFF 時は adjudicator の verdict 2 値・filter chain 件数・marker byte すべて導入前と等価） | `PR_ITERATION_OOS_ENABLED` | `false` | `=true` 厳密一致のみ有効。それ以外（未設定 / 空文字 / `True` / `TRUE` / `1` / `0` / `on` / typo）はすべて OFF に正規化（安全側 no-op） | **前提**: `PR_REVIEWER_ADJUDICATOR_ENABLED=true`（out-of-scope verdict は adjudicator が生成）。任意: `PR_ITERATION_OOS_ROUTE`（既定 `needs-decisions` / 本 spec では `design-reflow` / `spawn-issue` も `needs-decisions` に正規化）、`PR_ITERATION_OOS_NO_PROGRESS_LIMIT`（既定 `2` / 非数値・`<1` は `2` に正規化） | — | #437 |
| **Design PR Reviewer**（設計 PR (`claude/issue-<N>-design-<slug>`) に対する独立 Claude 設計レビュアを起動し、`docs/specs/<N>-<slug>/{requirements.md, design.md, tasks.md}` の **3 観点**（AC カバレッジ / design⇄tasks 整合 / Traceability）のみで approve / reject を判定し、`claude-review` commit status を publish + `needs-iteration` ラベルを付与/解消する。`claude-review` を必須 status check 化した consumer repo で発生する「設計 PR が永久 BLOCKED → admin-bypass merge に依存」事象を解消する。impl PR 用 Reviewer / #404 adjudicator とは **完全独立の処理経路** で、pr-reviewer.sh / adjudicator.sh のコード・env・ラベル運用に一切触らない。spec 本文は **head ブランチの git ref（`origin/<head_ref>`）** から取得し（#404 adjudicator と同方式 / 作業ツリーは base チェックアウト中で新規設計 PR の spec dir を持たないため / #433）、**spec 本文を 1 つも取得できない / spec dir 解決不能の場合は Claude を起動せず `claude-review = pending` に据え置く fail-closed**（空虚 approve の防止 / #433）。判定出力の parse / validate 失敗時のみ **保守的に approve に倒し** false-reject による永久 BLOCKED を回避する設計（人間運用の `awaiting-design-review` ラベルゲートと OR 条件で併存）。プロンプトテンプレ未配布（installer 未再実行）の repo では processor 全体を WARN 1 行で no-op return（claude 不起動 / status・ラベル不変 / 冪等再試行 / 永久 BLOCKED を作らない）。gate OFF 時は claude / gh / git 呼び出しゼロ・観測ログ diff ゼロで本変更前の opt-in 既定 OFF と完全に等価。**#432 で既定反転**: opt-in / 既定 OFF → **opt-out / 既定 ON**） | `DESIGN_REVIEWER_ENABLED` | `true`（**#432 で既定反転**） | `=false` 厳密一致のみ無効化（opt-out）。それ以外（未設定 / 空文字 / `True` / `TRUE` / `1` / `0` / `on` / typo）はすべて ON に正規化（安全側） | **前提**: `claude` CLI が watcher 実行環境に**インストール・認証済み**であること（既存必須ツール）。推奨: `DESIGN_REVIEWER_MODEL`（既定 `claude-sonnet-4-6`）、`DESIGN_REVIEWER_EXEC_TIMEOUT`（既定 `300` 秒）、`DESIGN_REVIEWER_PROMPT`（空なら内蔵 default `~/bin/design-review-prompt.tmpl`）、`DESIGN_REVIEWER_HEAD_PATTERN`（既定 `^claude/issue-[0-9]+-design-` / `PR_ITERATION_DESIGN_HEAD_PATTERN` と既定値共有 / 独立 env）、`DESIGN_REVIEWER_MAX_PRS`（既定 `5`）、`DESIGN_REVIEWER_OUTPUT_FORMAT`（`text` / `json` の 2 値、それ以外は `text` に正規化） | [Design PR Reviewer (#407 / #432)](#design-pr-reviewer-407) | #407, #432, #433 |
| **設計ルート fail-closed 検証**（design モード（PM → Architect → PjM）の Claude 実行が rc=0 で終了した直後に「設計 PR が実際に作成されたか（head=`claude/issue-<N>-design-<slug>` の open PR）／`awaiting-design-review` へラベル遷移したか」を検証し、いずれも未確認なら silent success（rc=0 で `claude-claimed` 放置）ではなく **`claude-failed`** へ倒す。orchestrator がサブエージェント（PM / Architect）を `run_in_background` で起動して「完了通知を待ちます」と end_turn した場合、headless 1-shot 実行では subagent 完了の再 invoke が無いため設計成果物が commit されず設計 PR も作られないまま Issue がゾンビ化する事象（ae-mdm #39/#40）を検出する。併せて design orchestrator prompt に `run_in_background` 禁止・逐次実行の制約を明示。gh 取得失敗 / 入力破損は false-fail を避け安全側（成功扱い）に倒す） | `DESIGN_PR_VERIFY_ENABLED` | `true`（opt-out） | `=false` 厳密一致のみ無効化（検証 skip = 導入前と同一挙動）。それ以外（未設定 / 空文字 / `True` / `1` / typo）はすべて ON に正規化（安全側 = 検証する） | 前提なし（design モードでのみ発火 / gh 読み取りのみで副作用なし）。migration note: 既定 ON だが fail-closed するのは「設計 PR も awaiting-design-review 遷移も両方無い」場合のみのため、正常に設計 PR を作った run は影響を受けない | — | #446 |
...
3. **spec 本文取得（head ブランチの git ref）**: 設計成果物
   （`docs/specs/<N>-<slug>/{requirements,design,tasks}.md`）の本文を **head ブランチの git
   ref（`origin/<head_ref>`）** から取得します（`git cat-file -e ... && git show ...` /
   #404 adjudicator と同方式）。watcher の作業ツリーは base にチェックアウトされており新規
   設計 PR の spec dir は作業ツリーに存在しないため、作業ツリーから読むと全観点が `(none)`
   となり空虚 approve（ラバースタンプ）になる不具合がありました（#433 で修正）。head ref から
   取得することで、未マージの spec 本文も正しくレビュー入力にできます。
4. **fail-closed（spec 本文取得不能時は approve しない / #433）**: spec dir を head ブランチ
   から解決できない、または 3 ファイルの本文を **1 つも取得できない** 場合は、**Claude 判定を
   起動せず** `claude-review` を `pending` に据え置きます（approve を publish しない）。
   この経路では判定 marker / 判定コメントを投稿せず、次サイクルで再試行されます。spec dir は
   解決済みなのに本文を取得できない場合は WARN ログを 1 行出力し、ログが健全に見えたまま空虚
   approve を見逃す罠を可視化します（#433 Req 2 / 3 / NFR 3.1）。
5. **判定**: spec 本文取得に成功した場合のみ、内蔵 `~/bin/design-review-prompt.tmpl` で設計
   成果物を Claude に渡し、3 観点（AC カバレッジ / design⇄tasks 整合 / Traceability）のみで
   `approve` / `reject` を判定させます。**判定出力の parse 失敗 / schema validate 失敗時**は
   保守的に `approve` に倒します（Req 2.4 / false-reject による永久 BLOCKED を回避）。この
   保守的 approve は **spec 本文取得が成功している場合に限定**され、spec 本文取得不能は前項の
   fail-closed が適用されます（#433 Req 4）。スタイル違反 / 命名 / typo / 表記揺れを理由とした
   `reject` は absolute prompt 規約で禁止されます（Req 2.6）。
6. **`claude-review` status publish**:
   - `verdict=approve` → `claude-review = success`
   - `verdict=reject` → `claude-review = failure`
   - claude exec 失敗 / timeout / **spec 本文取得不能（fail-closed / #433）** →
     `claude-review = pending` のまま据え置き（Req 3.3 / 次サイクルで再試行）
7. **`needs-iteration` ラベル**:
   - `verdict=reject` → `needs-iteration` を付与（既付与で冪等）
   - `verdict=approve` → `needs-iteration` を解消（未付与で冪等）
8. **判定コメント投稿**: 判定結果サマリ（VERDICT + 3 観点 reason）を hidden marker 付きで
...
- 本 Reviewer が claude exec 失敗 / timeout / **spec 本文取得不能（fail-closed / #433）** →
  `claude-review = pending` 据え置き → 次サイクルで再試行（人間運用は
  `awaiting-design-review` ラベルで並行進行）

### トレードオフ・既知の制約

- **判定精度 100% の達成は不可**（LLM 判定の本質的限界）: 本 Reviewer は判定出力の parse 失敗 /
  schema validate 失敗時に **保守的に approve に倒す** 設計で false-reject による永久 BLOCKED を
  構造的に回避しますが、これにより false-approve（本来 reject すべき設計を approve）のリスクが
  残ります。人間の `awaiting-design-review` ラベルゲートと OR 条件で併存することで、false-approve
  は人間レビュア側で補完します。なお **spec 本文を取得できないケースは保守的 approve の対象外**
  であり、Claude 判定を起動せず `pending` に据え置く fail-closed が適用されます（#433）。これに
  より「レビュー対象を読めていない設計 PR が無レビューで auto-merge される」空虚 approve は構造的
  に防止されます。
