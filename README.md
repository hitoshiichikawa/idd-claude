...
| **Context Map for per-task agents**（per-task Implementer / Reviewer 起動直前に `_Boundary:_` + design.md File Structure Plan から `context-map.md` を決定論的に生成し、後段 prompt に inline embed して広域 grep / glob を抑止） | `CONTEXT_MAP_ENABLED` | `false` | `=true` 厳密一致のみ有効。それ以外（未設定 / `True` / `1` / `yes` / typo）はすべて `false` 等価 | **前提**: `PER_TASK_LOOP_ENABLED=true`（本機能は per-task ループ内でのみ動作。単一実装パスでは未注入） | [Context Map for per-task agents (#313)](#context-map-for-per-task-agents-313) | #313 |
...
inline embed する opt-in 機能です。fresh session で起動される per-task agent が広域 grep / glob
を反復することによる token 消費を抑制し、当たり付け済みの候補ファイル列挙から実装・レビュー
に入れるようにします。

> **注**: 親 Issue [#313](https://github.com/hitoshiichikawa/idd-claude/issues/313)。
> 本機能は **opt-in（既定 OFF）**。`CONTEXT_MAP_ENABLED` 未指定 / `=true` 以外 / または
> `PER_TASK_LOOP_ENABLED=true` でない実行では、本機能導入前と user-observable に完全同一の
> 挙動（context-map.md 非生成 / prompt 注入なし）が維持されます（Req 1.2, 1.4 / NFR 1.1）。

### opt-in 手順

cron / launchd の `REPO=... REPO_DIR=...` に `PER_TASK_LOOP_ENABLED=true` と
`CONTEXT_MAP_ENABLED=true` の **両方**を追加します（前者が未設定だと本機能は no-op）。
...
| 変数 | デフォルト | 推奨 | 用途 |
|---|---|---|---|
| `CONTEXT_MAP_ENABLED` | `false` | 試運転時のみ `true` | 本機能の opt-in gate。`=true` 厳密一致のみ有効（`True` / `1` / `yes` / typo 等は `false` 等価 / Req 1.3）。`PER_TASK_LOOP_ENABLED=true` でない実行では値に関わらず常に no-op（Req 1.4） |
...
- **既定では従来挙動が維持されます**（NFR 1.1）。`CONTEXT_MAP_ENABLED` を設定しない限り、
  または `PER_TASK_LOOP_ENABLED=true` でない実行では、watcher の prompt 内容・stage 遷移・
  exit code・ログ出力は本機能導入前と **差分等価**です
- 既存 env var 名 / ラベル名 / cron 登録文字列は変更されません（NFR 1.2〜1.4）
- 過去 spec への retrofit 生成は行いません（Out of Scope）。`CONTEXT_MAP_ENABLED=true` を
  設定した時点から、対象 Issue の新規 per-task 起動でのみ生成されます
