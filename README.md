...
| `claude-picked-up` | Claude Code 実行中（impl 系では Stage A → Reviewer round=1 → 必要なら Stage A' → Reviewer round=2 → PjM の全 stage で維持） | Claude |
| `ready-for-review` | 実装 PR 作成完了 | Claude（PjM implementation モード後 / impl 系では Reviewer の approve を経て初めて遷移） |
| `skip-triage` | Triage をスキップ | 人間（任意） |
| `claude-failed` | 自動実行停止中（impl 系では Stage A 失敗 / Stage A' 失敗 / Reviewer 異常終了 / Reviewer round=2 reject も含む） | Claude（エラー連続時） |
...
auto-dev (起票)
   ...
   └─ claude-picked-up  ─→ Stage A → Stage B → ... 同上 ... → ready-for-review / claude-failed
```

`claude-picked-up` は impl 系モードの Stage A 開始時に付与され、PjM が `ready-for-review`
...
> Per-task implementation loop（Phase 2）/ Debugger サブエージェント（Phase 3）/
> Feature Flag Protocol（Phase 4）は別 Issue として分離します。
