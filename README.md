...
> **strict 関連 env は本 spec では実装されない**: `SECURITY_REVIEW_MODE` /
> `SECURITY_REVIEW_BLOCK_SEVERITY` / `SECURITY_REVIEW_BLOCK_LABEL` 等の env 名は **別 Issue
> #281（strict 拡張）で確定** します。本 spec の `sec_check_strict_request` は `=advisory`
> 以外の値が渡された場合に WARN を 1 行記録した上で advisory 固定の挙動を保ちます
> （sticky なフォールバック / Req 5.3）。
...
- **advisory 固定 / マージブロックなし**: 本 spec の Security Review Processor は **検出
  結果の severity に関わらずマージ阻害ラベルを付与しません**。strict モード（severity 閾値
  ベースの `needs-iteration` 自動付与）は本 spec では **未実装** で、
  [Issue #281](https://github.com/hitoshiichikawa/idd-claude/issues/281) として **別 Issue
  に分割済み** です。本 spec では env 予約・ラベル定義・閾値 env 名のいずれも導入しません
  （別 Issue で段階導入される際にあらためて設計される予定）。
