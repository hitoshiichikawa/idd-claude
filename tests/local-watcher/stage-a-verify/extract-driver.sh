...
declare -A _SAV_EXPECTED=(
  ...
  #   - tasks-backtick-prefix-prose.md (#160 round=2 / Req 5.3): backtick 内の
  #     冒頭が keyword 以外（例: `cd app && ./gradlew test`）の場合は、抽出関数
  #     側の行頭一致 (`index == 1`) により SKIPPED 扱いとなる。
  ...
  ["tasks-backtick-prefix-prose.md"]=""
)
