...
## PR Iteration / impl-resume round 内 self-commit 規律（Issue #435）

PR Iteration（`needs-iteration` ラベルが付いた PR を round 単位で再実装する経路）や
impl-resume で Developer が実装変更を行ったときは、**その round 内で Developer 自身が
意味のある commit メッセージで commit + push する責務を負う**。watcher の auto-recovery
commit に commit を肩代わりさせて運用しないこと。本節は前節「impl-resume / tasks.md 進捗
追跡規約」の既存 commit 温存規律（`git reset` / `git rebase` 禁止）を前提とし、その意味を
変えずに「round 内で生じた変更を誰がどう commit するか」を補足するものである。

- **round 内 self-commit の責務**: 1 round 内でコード / テスト / docs に実装変更を加えたら、
  その round が終わる前に Developer 自身が `git add` → `git commit` → `git push` まで完了
  させる（AC 1.1）。「変更だけ残して commit せずに round を終える」運用をしない
- **Conventional Commits で作業意図を残す**: round 内 self-commit のメッセージは
  Conventional Commits 規約（`feat` / `fix` / `test` / `docs` / `refactor` / `chore`）に従い、
  何をなぜ変えたかが読み取れる説明にする（AC 1.2）。小さな単位に分割してよい
- **auto-recovery commit は保険であり常用しない**: watcher の auto-recovery commit
  （`docs(specs): recover uncommitted round-N output (auto)` / soft-fail 系の
  `partial round-N output before quota cutoff (auto-recovered)`）は、round が中途で異常終了
  して dirty な作業ツリーが残ったときに**取りこぼしを防ぐための保険**である。Developer の
  通常 commit 経路の代替として常用しない（AC 1.3）。auto-recovery 任せにすると commit 履歴が
  作業意図を反映しない `recover uncommitted ... (auto)` ばかりになり、後段レビューの判断材料が
  失われる
- **既存 commit 温存規律と矛盾しない**: 本 self-commit はあくまで**追加 commit**として積む。
  前節同様 `git reset` / `git rebase` / branch 切替で既存 commit を巻き戻さない（AC 1.4 /
  1.5）。打ち消しが必要な場合は追加 commit で打ち消す
- **進捗マーカー commit との関係**: tasks.md の `- [ ]` → `- [x]` マーカー更新は前節どおり
  `docs(tasks): mark <task-id> as done` の別 commit で行う。実装の self-commit と marker
  commit を混在させない

> 補足: round 内 self-commit を Developer 自身が実行しても HEAD が進むため、watcher 側の
> no-progress 判定（round 開始時 HEAD と round 終了時 HEAD の SHA 比較）は「進捗あり」として
> 連続カウンタをリセットする。auto-recovery 経由で commit された round も同様に HEAD が進む
> ため進捗ありとして扱われる（誤った premature `claude-failed` 昇格は発生しない）。本規律は
