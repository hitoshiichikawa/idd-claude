...
# Issue #388: am_enable_auto_merge_for_pr の rc=0 path で amm_save_pending（merged 検知用の
# pending state 登録）を呼ぶようになった。本テストでは extract_function で 1 関数のみを
# 読み込んでいるため `amm_save_pending` は unbound になる。auto-merge.sh 側で
# `declare -F amm_save_pending` でガードしているため、stub 未提供のまま呼び出されることは
# 想定されないが、Section 3 の sn_notify 検証で同一 path が走るタイミングで pending 登録の
# 引数を観測したいので stub を提供する。
...
# Issue #388 Req 1.1, 1.3: result=success → result=armed に変更（merge 完了との誤読を防ぐ）
assert_eq "#388 Req 1.1: armed callsite は result=armed を渡す（result=success ではない）" "armed" "$SN_NOTIFY_LAST_RESULT"
# detail にも armed 明示文言を含める（Req 1.3）
case "$SN_NOTIFY_LAST_DETAIL" in
  *"armed (squash on green checks)"*)
    echo "PASS: #388 Req 1.3: detail に「armed (squash on green checks)」を含み運用者の誤読を防ぐ"
    PASS_COUNT=$((PASS_COUNT + 1))
    ;;
  *)
    echo "FAIL: #388 Req 1.3: detail に armed 明示なし: $SN_NOTIFY_LAST_DETAIL"
    FAIL_COUNT=$((FAIL_COUNT + 1))
    ;;
esac
# Issue #388 Req 2.1: armed 成功直後に amm_save_pending を呼んで pending state 登録
assert_eq "#388 Req 2.1: armed 成功時に amm_save_pending が呼ばれる" "1" "$AMM_SAVE_PENDING_CALL_COUNT"
assert_eq "#388 Req 2.1: amm_save_pending に pr_number=100 を渡す" "100" "$AMM_SAVE_PENDING_LAST_PR"
assert_eq "#388 Req 2.1: amm_save_pending に event_type=auto-merge-merged を渡す" "auto-merge-merged" "$AMM_SAVE_PENDING_LAST_EVENT_TYPE"
