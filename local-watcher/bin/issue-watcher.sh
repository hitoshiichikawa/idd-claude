...
# review-notes.md を書かせる。git diff $BASE_BRANCH..HEAD と round 情報を inline で渡す。
build_reviewer_prompt() {
  ...
  if [ -z "$diff_content" ]; then
    diff_content="(差分が取得できませんでした。Reviewer が Bash で git diff ${BASE_BRANCH}..HEAD を再取得してください)"
  fi
  local head_sha
