...
# ─── 前半（pt_ ヘルパー + prompt builder）— modules/per-task-loop.sh へ切り出し済み（#461）───
...
#   build_per_task_implementer_prompt / build_per_task_reviewer_prompt は
#   modules/per-task-loop.sh が定義する。後半 runner（run_per_task_* / pt_mark_* escalation）は
#   実行順序温存のため下記に残置（#462 で移動完了予定）。bash の遅延束縛のため順序問題なし。
