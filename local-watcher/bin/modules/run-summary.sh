...
#   stage-a-verify が SKIP された実行、scaffolding（.claude/agents / .claude/rules）欠落を
...
#   一時ファイルは使わない。変数は export しない（claude --print 子プロセスへの汚染防止）。
...
# Args: $1 = "ready-for-review" | "needs-iteration" | "claude-failed" | "hold"
# Req 7.1, 7.2
rs_set_result() {
