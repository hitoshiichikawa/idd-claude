#!/usr/bin/env bash
# slot-worker.sh — Phase C Slot Runner ヘルパー モジュール（#16 / #466 切り出し）
#
# 用途:
#   Phase C: Issue 入口並列化（worktree slot + dispatcher, #16）のうち、`_slot_run_issue`
#   本体を除くヘルパー関数群（ロガー / pre-claim filter / slot 失敗処理 / resume・slug 判定 /
#   claude-review ステータス publish）。`_slot_run_issue` 自体は次 issue #467 で本 module へ
#   移動完了予定のため、現時点では issue-watcher.sh に残置する。
#
#   関数一覧（計 24）:
