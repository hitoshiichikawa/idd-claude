...
**Scope clarification**: 本 Issue は **impl PR の merge gate 問題のみ**を解消する。`claude/issue-<N>-design-*`
PR は `review-notes.md` を持たず `claude-review` catch-up publish 経路が発火しないため、`claude-review`
を必須化すると設計 PR が永久 BLOCKED になる事象（ae-mdm で観測）は本 Issue では解消されない。
設計 PR ゲートは別 Issue として切り出すことを推奨し、本 Issue マージ後も設計 PR は人間 admin
merge / 当該 status を required 解除する運用が継続される。Q1 / README に同旨を明示する。
...
### Architecture Decision: claude-review publisher contention

#### Problem

`claude-review` commit status の publisher は 2 系統存在する:

- **catch-up 経路** (`process_claude_review_status_catchup` / `pr_publish_claude_status_from_branch`,
  `pr-reviewer.sh:1336-1380`): **毎 cycle・全 open PR** を走査し、head branch の `review-notes.md`
  の verdict（approve/reject）から `claude-review` を publish する（#374）
- **adjudicator 経路（本 Issue 新規）**: **per sha で 1 回**（Q3 dedup）、codex 指摘の裁定結果
  から `claude-review` を publish する

両者が同じ context に latest-wins で書く構造は、ae-mdm 設計レビュー（PR #405）で「adjudicator が
`failure` を publish した次 cycle で catch-up が `success` に上書きし、本機能の目的が無効化
される」「Reviewer が reject していても adjudicator が success に上書きできる（独立 Reviewer
判定の上書き）」の 2 つの実害が指摘された。`claude-review` が「独立 Reviewer の verdict」と
「codex 指摘の裁定」の 2 つの意味を抱え、どちらが gate かタイミング依存になる semantic 混線も
ある。

#### Options

| 観点 | 案 (a) catch-up 抑止 + Reviewer 先行優先（採用） | 案 (b) 別 context `adjudicator-review` |
|---|---|---|
| Req 3.2-3.4 「adjudicator が `claude-review` を publish」 | 充足 | **違反**（context 名変更を要件再編で吸収する必要） |
| consumer の branch protection 設定変更 | 不要（`claude-review` を引き続き required にする） | **必要**（新 context を required に追加） |
| catch-up の既存挙動への影響 | adjudicator 管轄 PR のみ defer（その他は無変更） | 無変更 |
| Reviewer reject の保持 | 保持（adjudicator は Reviewer reject を上書きしない） | 別 context なので独立に reject 保持 |
| 単一 status を単一 publisher が確定する原則 | adjudicator 管轄 PR では adjudicator が唯一の publisher | catch-up は `claude-review`、adjudicator は `adjudicator-review`（2 status で並列確定） |
| semantic 混線解消 | adjudicator 管轄 = codex 裁定を含む統合 verdict と再定義 | 「独立 Reviewer = `claude-review`」「codex 裁定 = `adjudicator-review`」と分離 |
| 実装変更点数 | 中（catch-up に suppression hook 1 つ、adj に Reviewer 読み込み 1 つ） | 中（新規 context publisher / consumer branch protection 文書） |

**採用案**: **案 (a) catch-up 抑止 + Reviewer 先行優先**。理由:

1. Req 3.2-3.4 は「adjudicator が `claude-review` を publish」と明示しており、案 (b) は要件再編
   （`adjudicator-review` 新規 context の導入）を要求する。本 PR 段階で要件再編は scope 外
2. consumer repo の branch protection 設定は本 Issue scope 外（Req 3.6）であり、案 (b) は
   全 consumer に「`adjudicator-review` を required に追加」という運用変更を強いる
3. case (a) は「adjudicator 管轄 PR では catch-up を defer + adjudicator が Reviewer verdict を
   先行優先」の 2 段で「単一 status を単一 publisher が確定する」原則を担保できる

#### Behavior contract

adjudicator 管轄 PR（gate ON 且つ adjudicator marker `<!-- idd-claude:pr-adjudicator sha=<sha> -->`
が PR コメントに存在）に対して:

1. **catch-up 抑止**: `process_claude_review_status_catchup` は当該 PR をループ内で skip する
   （新規 helper `pr_catchup_should_defer_for_adjudicator <pr_number> <sha>` が true を返したら
   `pr_publish_claude_status_from_branch` 呼び出しを skip）
2. **Reviewer 先行優先**: adjudicator は publish 直前に head branch の `review-notes.md` を読み、
   verdict が `reject` なら **legitimate 件数に依らず `claude-review = failure`** を publish
   する（Reviewer reject の意味を維持。「全部 excessive」判定でも success に上書きしない）
3. **failure 持続性**: adjudicator は per-sha で 1 回しか走らないが、catch-up suppression により
   「次 cycle で catch-up が success に書き戻す」事象が発生しない。adjudicator の `failure` は
   sha 不変の間維持される
4. **adjudicator 不発時の fallback**: adjudicator が exec 失敗 / rate-limit / timeout 等で
   publish できなかった sha については、catch-up suppression は当該 sha だけ解除される
   （adjudicator marker 不在のため `pr_catchup_should_defer_for_adjudicator` が false を返す）。
   この場合 catch-up が従来どおり `review-notes.md` の verdict を publish する。`PR_REVIEWER_ADJUDICATOR_FALLBACK_ON_FAIL`
   が `passthrough` 設定なら adjudicator は marker を投稿しない（catch-up が引き継ぐ）

#### Reviewer verdict precedence の境界

- Reviewer verdict の評価対象は head branch 上の `docs/specs/<N>-<slug>/review-notes.md` の
  最終行 `RESULT: approve|reject`。`review-notes.md` 不在 / `RESULT:` 行不在 / `approve` の
  いずれかなら adjudicator は legitimate 件数のみで verdict を決める
- Reviewer reject 検出時の挙動は Req 3.3 の境界補強（後述 Requirements Traceability で同 ID
  に precedence 追記）であり、Req 3.4「legitimate ≥1 で failure」とは整合的（Reviewer reject =
  failure を強化する方向）。Req 3.3「legitimate ゼロで success」については Reviewer reject を
  先行優先する例外を追加（requirements.md 側で軽微整合修正済み）

#### Latency / cost stacking

1 cycle あたりの直列実行は codex（最大 1800s）+ adjudicator（最大 300s）で増加する。
`PR_REVIEWER_MAX_PRS` / `PR_REVIEWER_ADJUDICATOR_EXEC_TIMEOUT` で上限を抑制し、README の
本節で latency stacking と flock 排他下での他処理待ちを明示する（Req 6 ドキュメント要求の
延長として README に集約）。
...
- `claude-review` status の publisher 競合（catch-up vs adjudicator）は **catch-up 抑止 +
  Reviewer verdict 先行優先**で「単一 status を単一 publisher が確定する」原則を担保する
  （後述「Architecture Decision: claude-review publisher contention」参照。latest-wins には
  依存しない）
...
- `local-watcher/bin/modules/pr-reviewer.sh`（**編集**） — 2 箇所:
  1. `pr_run_review_for_pr` の末尾、`pr_publish_codex_status` 直後に
     `adj_run_for_pr "$pr_number" "$sha" "$review_text" "$pr_url" "$head_ref" || adj_warn ...` を
     **1 行**追加。既存ラベル付与・status publish ロジックは**残置**（gate OFF 時の完全 no-op
     保証 / NFR 1.1）
  2. `process_claude_review_status_catchup` ループ内、`pr_publish_claude_status_from_branch`
     呼び出し直前に `pr_catchup_should_defer_for_adjudicator "$pr_number" "$sha" && continue`
     を **1 行**追加（gate ON で adjudicator marker 検出時のみ catch-up を skip / Architecture
     Decision: claude-review publisher contention）。新規 helper 関数本体（marker fetch + sha
     一致判定）は同ファイル末尾に **追記**
...
| 3.2 | adjudicator が claude-review を publish | `adj_publish_claude_status` + catch-up suppression via `pr_catchup_should_defer_for_adjudicator` | 既存 `pr_publish_claude_status` 再利用 + catch-up skip hook | B |
| 3.3 | legitimate ゼロ → claude-review = success（Reviewer reject 時は failure 優先） | `adj_apply_status_decision` + `adj_read_reviewer_verdict` | state="success" / Reviewer reject 時は "failure" | B |
| 3.4 | legitimate ≥1 → claude-review = failure | `adj_apply_status_decision` | state="failure" | B |
| 3.5 | Reviewer reject → claude-review = failure（legitimate ゼロでも） | `adj_apply_status_decision` + `adj_read_reviewer_verdict` | state="failure" 強制 | B |
| 3.6 | codex exec-failed でも legitimate ゼロかつ Reviewer reject 不検出なら success | adj_run_for_pr の空 review_text 処理 + Reviewer verdict 読み | flow branch | B |
| 3.7 | watcher は codex-review 必須化判定を持たない | 既存 `pr_publish_codex_status` のまま | 非干渉 | (既存) |
...
# 戻り値: 0=ok / 4=parse-count 不整合（reconcile 失敗、呼び出し元で全件 legitimate に倒す）
# 副作用: ## 指摘事項 見出し配下の bullet 行数と parse 件数を突合する reconciliation check を内蔵
#         （ae-mdm 設計レビュー #4 への対応 / 書式ドリフトによる silent 取りこぼし防止）。
#         件数不一致を検出した場合、stderr に WARN を出力し戻り値 4 を返す（findings_json は
#         空配列ではなく parse 済み subset を返す。fail-safe は呼び出し元 adj_run_for_pr が
#         担う）
...
# 副作用: publish 直前に head_ref から review-notes.md の Reviewer verdict を読み、reject の
#         場合は legitimate_count に依らず status=failure を publish する（Reviewer 先行優先 /
#         Architecture Decision: claude-review publisher contention）。review-notes.md 不在 /
#         RESULT 行不在 / approve のいずれかなら legitimate_count のみで verdict を決める
# 戻り値: pr_publish_claude_status の戻り値
adj_apply_status_decision()

# head branch 上の review-notes.md から Reviewer の最終 verdict を読む
# 入力: $1=head_ref（または絶対パス hint。実装は git show :path 形式で読む）
# 出力: stdout に "approve" | "reject" | "" のいずれか（空は不在 / RESULT 行不在）
# 戻り値: 0 固定（不在は空文字列として返す）
...
publish を上書きする。ラベルも `gh pr edit` の冪等性で吸収される（既付与の `--add-label` は
no-op、未付与の `--remove-label` も no-op）。

#### Component: `pr_catchup_should_defer_for_adjudicator`（新規）

| Field | Detail |
|---|---|
| Intent | adjudicator 管轄 PR で catch-up を skip し、`claude-review` status の 2 系統 publish 競合を解消する |
| Requirements | 3.2, 3.3, 3.4, 4.3 |

**Responsibilities & Constraints**

- 主責務: gate ON 且つ adjudicator marker（`<!-- idd-claude:pr-adjudicator sha=<sha> -->`）が
  当該 PR コメントに存在する場合のみ true（defer）を返す。gate OFF / marker 不在 / sha 不一致
  なら false を返す（catch-up 続行）
- gate OFF 時: 早期 return false（既存 catch-up 挙動を維持 / NFR 1.1）
- adjudicator が exec 失敗等で marker を投稿していない sha については **defer しない**
  （Architecture Decision: claude-review publisher contention の Behavior contract 4 参照）
...
# 副作用: なし（read-only: gh pr view --json comments のみ）
pr_catchup_should_defer_for_adjudicator()
```

**配置**: `process_claude_review_status_catchup` ループ内、`pr_publish_claude_status_from_branch`
呼び出し直前に `pr_catchup_should_defer_for_adjudicator "$pr_number" "$sha" && continue` を
1 行挟む。helper 関数本体は同ファイル末尾に追記する。
...
| `PR_REVIEWER_ADJUDICATOR_FALLBACK_ON_FAIL` | `passthrough` | claude 失敗時 fallback verdict | `legitimate` / `passthrough` の 2 値、それ以外は `passthrough` |
| `PR_REVIEWER_ADJUDICATOR_MAX_FINDINGS` | `50` | 1 PR あたり処理する指摘数上限（コスト抑制） | 非数値は既定 |

`PR_REVIEWER_ADJUDICATOR_FALLBACK_ON_FAIL` の意味:
- `passthrough`（**既定**）: claude 失敗時は **adjudicator 自体を実行しなかった**かのように扱い、
  既存 `pr_add_iteration_label` / `pr_publish_codex_status` の結果と既存 catch-up（独立 Reviewer
  verdict）をそのまま残す。adjudicator marker も投稿しないため、catch-up suppression は当該
  sha について発火せず、catch-up が `claude-review` を引き継いで publish する（impl PR では
  独立 Reviewer が既に approve 済みなら success が維持される / ae-mdm 設計レビュー #2「SPOF 緩和」
  への対応）
...
**既定変更の根拠**: 当初案では `legitimate` 既定としていたが、merge gate を adjudicator が単独
で握る構造では adjudicator 自身が SPOF 化し、その失敗で legitimate 既定が常時 block を引き起こす
（codex の SPOF を Claude の SPOF に付け替えただけ）。impl PR では独立 Reviewer サブエージェントが
別 context で並行に動くため、adjudicator 失敗時は **独立 Reviewer の verdict を尊重する経路**
（=passthrough）が運用安定性と安全性の両立点となる。明示的に `legitimate` を選びたい運用は
env で上書きできる。
...
adjudicator のエラー戦略は **`PR_REVIEWER_ADJUDICATOR_FALLBACK_ON_FAIL`** で 2 モードから選ぶ
（既定 `passthrough`）。claude exec 失敗 / JSON parse 失敗 / workspace 変更検出 / reconciliation
mismatch のいずれの場合も:

1. `passthrough`（**既定**）: adjudicator の publish / label 操作 / marker 投稿を **すべて skip**。
   既存 `pr_*` の結果（`codex-review` status / `needs-iteration` ラベル）と既存 catch-up
   （独立 Reviewer の `claude-review`）を温存。adjudicator marker 不在のため catch-up suppression
   は当該 sha で発火せず、impl PR では Reviewer の verdict が SPOF 影響を受けずに維持される
2. `legitimate`: 全 finding を legitimate と扱い、needs-iteration 維持 + `claude-review = failure`。
   marker は通常通り投稿（catch-up suppression を継続）
...
- **JSON parse 失敗**: WARN ログ + fallback モード適用 + decision コメント投稿（`## 自動裁定エラー`
  + `## 取得した raw 出力` 抜粋）
- **reconciliation mismatch（`adj_extract_findings` rc=4）**: WARN ログ + fallback モード適用。
  `adj_extract_findings` は codex 出力の `## 指摘事項` 配下 bullet 行数と parse 件数を突合し、
  不一致なら戻り値 4 を返す（書式ドリフトによる silent 取りこぼし防止 / ae-mdm 設計レビュー #4
  への対応）。`legitimate` 既定運用では fallback で全件 legitimate に倒し、`passthrough` では
  adjudicator skip
...
- **Reviewer reject 検出**: adjudicator は publish 直前に `adj_read_reviewer_verdict` で head
  branch の `review-notes.md` を読み、`reject` を検出した場合は legitimate 件数に依らず
  `claude-review = failure` を publish（Architecture Decision: claude-review publisher
  contention の Behavior contract 2）
...
### Q1. 設計 PR ゲートの併発課題（impl PR 限定スコープの明示）
...
なる事象がある（ae-mdm で観測）。本 Issue では **impl PR の adjudicator 化のみ**をスコープとし、
設計 PR ゲートは **別 Issue として切り出す**ことを推奨する。

**重要**（ae-mdm 設計レビュー #3 への対応）: 本 Issue を merge しても **設計 PR の `claude-review`
永久 BLOCKED 事象は解消されない**。設計 PR に対しては引き続き以下のいずれかで対処する必要が
ある:

- consumer repo の branch protection で `claude-review` を設計 PR に required 解除（ブランチ
  名 pattern マッチ運用 / GitHub branch protection の `Restrict pushes that create matching branches`
  と組み合わせ）
- 人間 admin merge（ae-mdm 現行暫定）

本 PR の Overview / README には「impl PR の merge gate 問題のみ解消、設計 PR は別 Issue で対応」
を明記する（README 新規節「PR Reviewer Adjudicator (#404)」の冒頭に limitation 段落を置く）。
