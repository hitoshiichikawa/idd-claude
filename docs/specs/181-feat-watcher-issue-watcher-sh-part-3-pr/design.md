...
- **`stage-a-verify` マニフェスト追加の取り扱い**: decision 2 で `impl-gates.sh` から独立分離するため、Part 1 の `_IDD_MODULES` に `stage-a-verify` を追加する必要がある。Part 1 マニフェストは `impl-gates` を含むが本 Part が分離する `stage-a-verify` を含まないため、本 Part の移動タスクでマニフェスト追加を伴う（Step3）。**Part 1 の impl-gates.sh から `sav_*` を抜く整合は Part 1 設計との突合せが必要**であり、PR 本文の確認事項に明記する
