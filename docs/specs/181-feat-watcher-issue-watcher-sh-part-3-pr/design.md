...
- モジュールローダ・マニフェスト・テスト互換ヘルパー・install.sh の modules コピーといった **分割基盤の新設**（これらは Part 1 #177 が所有する。本 Part は Part 1 基盤の上に乗る前提依存。後述 Migration Strategy）
...
- **既存テストは本体から awk 抽出 / grep する**: `pi_max_rounds_kind_test.sh` は `pi_resolve_max_rounds` / `pi_read_no_progress_streak` を本体から awk 抽出、`tests/local-watcher/stage-a-verify/extract-driver.sh` は `stage_a_verify_extract_command` を本体から awk 抽出する。**抽出元パスが変わると壊れる**ため、Part 1 の「Test Compatibility Layer（案 b）= `IDD_SOURCE_FILES` 集合走査」を踏襲する

### Architecture Pattern & Boundary Map

**採用パターン**: Part 1 の Entry-point + Sourced Library Modules を踏襲。本 Part は新パターンを導入せず、Part 1 マニフェストに登録済みの 4 モジュール（後述 decision で命名・粒度を確定）へ、本体から該当プレフィックスの関数定義を移すだけである。
...
- **新規コンポーネントの根拠**: 本 Part は新規コンポーネントを追加しない（モジュールローダ・マニフェスト・テスト互換ヘルパー・install.sh コピーは Part 1 所有）。本 Part は Part 1 が用意した空モジュール（または既存モジュール）へ関数定義を移すだけである
...
Part 1 が確定した `local-watcher/bin/modules/` 配下に、本 Part が関数定義を移す **3 ファイル**を以下に明示する（decision 1〜3 で命名・粒度を確定済み）。`promote-pipeline.sh` は Part 1 マップにも存在するモジュール名で、本 Part は `pp_*` + `po_*` をそこへ集約する。`pr-iteration.sh` も Part 1 マップに存在。`stage-a-verify.sh` は本 Part の decision 2 で Part 1 の `impl-gates.sh` から分離した新規モジュール（後述 decision 参照）。
...
└── modules/                    # Part 1 が新設。本 Part は以下 3 ファイルへ関数定義を移す。
...
### Part 1 が所有し本 Part が前提依存するファイル（本 Part では新規作成・編集しない）
- `local-watcher/bin/modules/` ディレクトリと **モジュールローダ / マニフェスト**（`issue-watcher.sh` 内の `_IDD_MODULES` 配列に `promote-pipeline` / `pr-iteration` / `stage-a-verify` が登録されていること。decision 2 で `stage-a-verify` をマニフェストへ追加する必要がある旨は確認事項として記録）
- `local-watcher/test/lib/extract.sh`（`IDD_SOURCE_FILES` 集合走査ヘルパー / `extract_function` / `idd_grep_sources`）
- `install.sh` の modules コピー行（`copy_glob_to_homebin "$LOCAL_WATCHER_DIR/bin/modules" "*.sh" "$HOME/bin/modules" --executable`）
...
- `local-watcher/bin/modules/promote-pipeline.sh` — `pp_*` + `po_*` + `process_promote_pipeline` を受け入れ（Part 1 が雛形を用意済みの前提。本 Part が中身を充填）
- `local-watcher/bin/modules/pr-iteration.sh` — `pi_*` + `build_recovery_hint` + `process_pr_iteration` を受け入れ
- `local-watcher/bin/modules/stage-a-verify.sh` — `sav_*` + `_sav_*` + `stage_a_verify_*` を受け入れ（**新規モジュール**。decision 2）
- `local-watcher/bin/issue-watcher.sh` の `_IDD_MODULES` マニフェスト — `stage-a-verify` を追加（decision 2 に伴う。Part 1 マニフェストに無いため。後述 Open Questions 参照）
...
| 6.1 | `local-watcher/test/` 全テスト成功 | Test Compat (extract.sh / Part 1) | `IDD_SOURCE_FILES` 集合走査 | 既存テスト互換戦略 |
| 6.2 | `tests/local-watcher/` 全テスト成功 | Test Compat (extract-driver) | modules 解決 | 既存テスト互換戦略 |
| 6.3 | 個別関数抽出が移動先に依らず解決可能 | Test Compat (extract.sh / Part 1) | 全 SOURCE 走査 | 既存テスト互換戦略 |
| 6.4 | 解決不能を成功扱いで隠さずテスト失敗化 | Test Compat (extract.sh / Part 1) | 抽出失敗→exit 非0 | Error Handling |
...
- **被テスト最多モジュールの一翼**: `stage_a_verify_extract_command` は `tests/local-watcher/stage-a-verify/extract-driver.sh` が awk 抽出する。Part 1 のテスト互換ヘルパー（`IDD_SOURCE_FILES` 集合走査）経由で移動先に依らず解決される（Req 6.3）
...
- マニフェスト `_IDD_MODULES` に `stage-a-verify` を追加する（decision 2。Part 1 マニフェストに `impl-gates` はあるが `stage-a-verify` 独立分離分が無いため。後述 Open Questions）
...
- **Test Resolution Errors（Part 1 所有 / Req 6.4）**: 共通抽出ヘルパー（`extract_function`）が関数を解決できない場合の非0 exit は Part 1 が提供する。本 Part は移動先 module を `IDD_SOURCE_FILES` 集合が走査することで Req 6.3 / 6.4 を満たす状態を維持するだけ
...
- **Unit Tests**:
  1. `tests/local-watcher/stage-a-verify/extract-driver.sh` — `stage_a_verify_extract_command` が stage-a-verify.sh（移動先）から `IDD_SOURCE_FILES` 集合走査経由で抽出でき全 fixture が pass する（Req 3.x / 6.2 / 6.3）
  2. `local-watcher/test/pi_max_rounds_kind_test.sh` — `pi_resolve_max_rounds` / `pi_read_no_progress_streak` が pr-iteration.sh から抽出解決でき従来どおり pass する（Req 1.x / 6.1）
  3. `local-watcher/test/pi_detect_quota_soft_fail_test.sh` — `pi_detect_quota_soft_fail` が pr-iteration.sh から抽出解決でき pass する（Req 1.x / 6.1）
...
- **Integration Tests**:
  1. `local-watcher/test/repo_prefix_log_test.sh` — `pp_log` / `pi_log` / `sav_log` の `[REPO]` prefix が移動先 3 module を横断して `idd_grep_sources` で解決される（Req 5.3 / 6.1 / 6.3）
...
- **Part 1 / Part 2 への前提依存（Req 8.2）**: 本 Part は Part 1 が新設する `modules/` ディレクトリ・モジュールローダ・マニフェスト・テスト互換ヘルパー（`IDD_SOURCE_FILES`）・install.sh コピー行が **存在することを前提**とする。Part 1 未実装（`modules/` 未存在）の状態で本 Part 単独を merge すると、移動した関数がロードされず watcher が動かない。したがって **本 Part の実装着手は Part 1 merge 後**を前提依存とする（PR 本文の確認事項に明記）。Part 3 単独完了時点では「4 群の関数定義を本体から除去した」状態（Req 8.2）に到達するが、本体 1,000 行以下（Req 8.1）の達成は Part 1 / Part 2 完了を前提とする
- **段階的移動の原則**: 一括移動で全テストが赤になる事故を避けるため、tasks.md は「各タスク完了時点で shellcheck + 全テストが緑」を保てる順序にする。Part 1 のテスト互換ヘルパー（`IDD_SOURCE_FILES` = 本体 + modules/ を走査）が既に導入済みのため、関数が本体に残った状態でも移動後でも同一に解決でき、各移動タスクをテスト緑のまま遷移させられる
- **`stage-a-verify` マニフェスト追加の取り扱い**: decision 2 で `impl-gates.sh` から独立分離するため、Part 1 の `_IDD_MODULES` に `stage-a-verify` を追加する必要がある。Part 1 マニフェストは `impl-gates` を含むが本 Part が分離する `stage-a-verify` を含まないため、本 Part の移動タスクでマニフェスト追加を伴う（Step3）。**Part 1 の impl-gates.sh から `sav_*` を抜く整合は Part 1 設計との突合せが必要**であり、PR 本文の確認事項に明記する。なお実装着手時には、Part 1 が実際に実装したマニフェスト配列 `_IDD_MODULES` の **要素集合と定義順** を確認したうえで `stage-a-verify` を追加し、要素の欠落・重複による source ロードエラー（= モジュール未ロード → 関数未定義事故）を起こさないよう同期する。関数定義順そのものは source-before-execution（前述 Architecture / Components 節）によりランタイム挙動へ影響しないが、マニフェスト要素の過不足はロード可否に直結するため、着手時点の Part 1 実態突合せを必須とする
...
- **副作用と Part 1 整合**: Part 1 の `impl-gates.sh` は当初 `sav_*` も含む想定だった。本 decision により Part 1 の `impl-gates.sh` から `sav_*` 群が抜け、代わりに `stage-a-verify.sh` が新設される。Part 1 の `_IDD_MODULES` マニフェストに `stage-a-verify` を追加する必要がある（本 Part の Step3 で実施）。
