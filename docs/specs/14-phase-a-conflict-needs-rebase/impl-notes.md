...
## レビュー指摘対応（PR #24 コメント起点）

初回レビュー（PR #24 コメント https://github.com/hitoshiichikawa/idd-claude/pull/24#issuecomment-4311021936 ）で指摘された High/Medium 項目を本 follow-up commit で対応:

- **H-1**: `repo-template/.github/scripts/idd-claude-labels.sh` に `needs-rebase` を同期
  - 消費者 repo で Phase A ラベルが作成されない問題を修正
- **H-2**: head branch / fork PR フィルタを追加
  - 新 env `MERGE_QUEUE_HEAD_PATTERN`（デフォルト `^claude/`、jq `test()` 互換 regex）
  - head repo owner が base repo owner と一致する条件も同時適用（fork PR 除外）
  - requirements.md に AC 1.6, 1.7 を追加
  - 人間の手書き PR / fork PR を自動 rebase の対象外にする（`gh pr list` が全 approved PR を返すことへの防衛）
- **H-3**: README に Branch Protection「Dismiss stale pull request approvals」設定との干渉警告を追加
  - approve dismiss のリスクと、Phase D (#17) 導入後の位置付けを明記
- **M-1**: README Phase A Migration Note に `$HOME/bin/issue-watcher.sh` の再配置手順を追記
  - `install.sh --local` または手動 `cp`
  - Phase A に限らず watcher を変更するすべての PR 共通の注意である旨も明記

### フィルタ動作の dry-run 確認

```bash
echo '[
  {"isDraft":false,"reviewDecision":"APPROVED","headRefName":"claude/issue-14-foo","headRepositoryOwner":{"login":"hitoshiichikawa"}},
  {"isDraft":false,"reviewDecision":"APPROVED","headRefName":"feature/human-work","headRepositoryOwner":{"login":"hitoshiichikawa"}},
  {"isDraft":false,"reviewDecision":"APPROVED","headRefName":"claude/issue-99-bar","headRepositoryOwner":{"login":"forkuser"}}
]' | jq --arg pattern '^claude/' --arg owner 'hitoshiichikawa' \
  '[.[] | select(.isDraft == false) | select(.reviewDecision == "APPROVED") | select(.headRefName | test($pattern)) | select((.headRepositoryOwner.login // "") == $owner)] | length'
# => 1  （claude/issue-14-foo のみ残り、human-work と forkuser が除外）
```

### 未対応（別 Issue で対処）

- **M-2** `needs-rebase` 付き PR の自動 re-check ループ
- **M-3** `--limit 50` 固定の overflow 精度改善
- **L-1** shellcheck 実行環境整備
- **L-4** AC 1.3「終端状態ラベルに準ずる」表現の曖昧さ（新規 1.6, 1.7 は具体化済、1.3 自体は #20 Phase 1 Reviewer で機械検出可能な領域）
