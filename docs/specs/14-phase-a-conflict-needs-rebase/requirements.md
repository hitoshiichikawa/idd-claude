...
6. If 処理対象 PR の head branch 名が Merge Queue Processor の head branch pattern（`MERGE_QUEUE_HEAD_PATTERN`、デフォルト `^claude/`）に合致しない, the Merge Queue Processor shall その PR を対象から除外する（人間が手書きした PR への意図しない自動 rebase を防ぐ）
7. If 処理対象 PR の head repo owner が base repo owner と異なる（= fork からの PR）, the Merge Queue Processor shall その PR を対象から除外する（origin にローカル参照が無いため自動 rebase 不可、かつ force push 権限も持たない）
