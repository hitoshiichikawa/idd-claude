...
- [ ] 2.2 `sec_build_marker` を実装（`<!-- idd-claude:security-review sha=<sha> kind=<kind> -->` を stdout 出力）
  - 引数: `$1 = sha`, `$2 = kind`（`security-review` / `scan-failed`）
  - 末尾改行なし
  - _Requirements: 3.4, 6.1, 6.4_
- [ ] 2.3 `sec_resolve_mode` を実装（`SECURITY_REVIEW_MODE` を読み advisory を返す）
  - `advisory` → そのまま `advisory` を stdout
  - `strict` → WARN ログ後 advisory fallback（本 spec では strict 未実装、env 予約のみ）
  - 未設定 / 空 / 不正値 → 安全側 advisory
...
- [ ] 3.2 `sec_post_review_comment` / `sec_post_error_comment` を実装
  - 本文末尾に `sec_build_marker` で marker を埋め込む
  - `sec_post_error_comment` は冒頭に `## セキュリティレビュー結果（実行エラー）` 見出し
  - `sec_post_error_comment` は `sec_already_processed` で `(sha, kind)` 重複を事前判定し既存なら skip
  - 投稿失敗時は WARN + rc=1
  - _Requirements: 2.6, 3.1, 3.2, 3.4, 6.1, 6.2, 6.4_
- [ ] 3.3 `sec_run_review_for_pr` を実装（1 PR 分のスキャン統括）
  - 重複判定（`kind=security-review` marker 既存なら rc=2 で skip）
  - prompt / cmd template 展開（`SECURITY_REVIEW_CLAUDE_CMD` プレースホルダ置換、shell metacharacter 検査）
  - `sec_execute_security_review` 呼び出し → 結果トークン分岐
  - 成功 + 出力非空 → `sec_post_review_comment` 投稿
  - 検出 0 件判定（出力テキストが「指摘なし」または空配列相当）→ コメント投稿せず log 1 行（安全側既定 / Req 3.3）
  - 失敗系（exec rc != 0 / workspace modified / 出力空）→ `sec_post_error_comment` 投稿
  - _Requirements: 2.2, 2.4, 2.6, 3.1, 3.2, 3.3, 3.4, 6.1〜6.4_

- [ ] 4. entrypoint `process_security_review` を実装し、issue-watcher.sh に配線
- [ ] 4.1 `process_security_review` を実装（opt-in gate / mode 解決 / 候補列挙 / truncate / ループ / サマリ）
  - 早期 return: `[ "${SECURITY_REVIEW_ENABLED:-false}" != "true" ] && return 0`（1 行ログ）
  - サイクル開始サマリ: `mode=<mode> max_prs=N git_timeout=Ns exec_timeout=Ns head_pattern=...`
  ...
  - サイクル終了サマリ: `reviewed=N skip=N fail=N errored=N overflow=N`
  - 最後に保険で `git checkout "$BASE_BRANCH"` で復帰
  - return 0 固定（dispatcher fail-continue）
  - _Requirements: 1.1, 1.2, 1.3, 1.4, 2.1, 2.5, 5.1, 5.4, 5.5, NFR 1.1, NFR 3.1_
- [ ] 4.2 `issue-watcher.sh` Config ブロックに env var 群を追加 (P)
  ...
  - 9 個の env var を `${VAR:-default}` 形式で解決（design.md「Environment Variables」表）
  - 既存 `_idd_flag` 正規化ループには **含めない**（opt-in 機能のため）
  - 既存 env var の名前・既定値・既存 Config ブロックは変更しない
  - _Requirements: 1.1, 5.5, NFR 1.1, NFR 1.2_
...
- [ ] 5. README.md にドキュメント追記（同一 PR 内で実施 / NFR 6.2）
  ...
  - 含める内容: 概要 / advisory のみ初期実装 / 既定 OFF の旨 / 動作フロー / 環境変数表 / 利用例 cron 行 / 既知の制約（strict は後続 Issue / `security-notes.md` 不採用）/ Migration Note（既存ユーザー向け）
