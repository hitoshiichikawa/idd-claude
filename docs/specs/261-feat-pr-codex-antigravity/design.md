...
| 4.2 | env 実行コマンド呼び出し + stdout 収集 | `pr_execute_review_command` | stdout キャプチャ | 4 |
| 4.3 | `{BASE}` プレースホルダ置換 | `pr_substitute_placeholders` | string 置換 | 4 |
| 4.4 | レビューテキストを PR に 1 回コメント | `pr_post_review_comment` + marker `kind=review` | hidden marker | 4 |
| 4.5 | 実行失敗 → エラーコメント + 中止 | `pr_execute_review_command` rc != 0 + `pr_post_error_comment` `kind=exec-failed` | hidden marker | 4 |
| 5.1 | キーワードマッチで `needs-iteration` 付与 | `pr_detect_iteration_keyword` + `pr_add_iteration_label` | `gh pr edit --add-label` | 5 |
...
- **Postconditions**: `process_pr_reviewer` は **常に 0 を返す**（dispatcher fail-continue 慣行）。副作用は `gh pr comment` / `gh pr edit --add-label` のみで、PR の body 編集や branch 操作は副作用として残さない（head branch checkout はサブシェル内で行い `trap` で `BASE_BRANCH` に戻す）
- **Invariants**: 同一 `(pr_number, sha, kind)` に対し marker が既存なら副作用ゼロ（NFR 4.1）

##### State / Marker Contract（hidden HTML comment 形式）

```
<!-- idd-claude:pr-reviewer sha=<headRefOid> kind=<review|conflict-tool|not-installed|not-authenticated|exec-failed> tool=<codex|antigravity|none> -->
```
...
| ツール check | `command -v $tool` / `<tool> auth status`（env で override 可） | AC 3.1 / 3.2 | exit code を直接利用 |
| レビュー実行 | `eval "$resolved_cmd"`（注: env で運用者が定義した文字列を実行するため CLAUDE.md の禁止事項に抵触しないよう、`$resolved_cmd` の各値は env から来る既知の運用者入力に限定。コマンド連結を許容する設計判断は Decisions 節を参照） | AC 4.2 | 非ゼロ → エラーコメント |
| head 取得 | `git fetch origin <head_ref>` + `git checkout -B <head_ref> origin/<head_ref>` | AC 4.1 | サブシェル + trap で BASE_BRANCH 復帰 |
...
| `PR_REVIEWER_CODEX_CMD` | `codex review --base {BASE}` | codex 実行コマンドテンプレート | 文字列。プレースホルダ `{BASE}` / `{HEAD}` / `{PR}` 置換可 |
| `PR_REVIEWER_ANTIGRAVITY_CMD` | `antigravity review --base {BASE}` | antigravity 実行コマンドテンプレート | 同上 |
| `PR_REVIEWER_CODEX_AUTH_CMD` | `codex auth status` | codex 認証チェックコマンド（空文字なら check skip） | 文字列。終了コード 0 で認証 OK 判定 |
| `PR_REVIEWER_ANTIGRAVITY_AUTH_CMD` | `antigravity auth status` | 同上 antigravity 版 | 同上 |
| `PR_REVIEWER_ITERATION_PATTERN` | `(?i)(needs[- ]iteration\|要修正\|fix\s+required\|please\s+fix\|修正してください)` | キーワード検出用 ERE（grep -E） | jq の test() / grep -E 互換ベース |
...
- 本機能は **`eval "$resolved_cmd"` を使う**（env で運用者が定義した実行コマンドテンプレートを展開実行する）ため、env var を信頼できる経路（cron / launchd entry / 運用者の shell rc）からのみ受け取る前提に依存する。Issue / PR の本文・コメント値は `{BASE}` / `{HEAD}` / `{PR}` 置換の **値側**に到達しうるが、いずれも GitHub branch 命名規約（`^[A-Za-z0-9._/-]+$` 相当）と整数 PR 番号に制限されており、shell metacharacter 注入は事実上発生しない
- それでも防御的設計として、`pr_substitute_placeholders` は置換結果に `;` `|` `&` `$(` などのメタ文字が混入していないかを scan し、混入を検出した場合は当該 PR を skip + WARN（運用者が予期しない GitHub 側仕様変更へのガード）
...
**採用**: `PR_REVIEWER_<TOOL>_CMD` 形式（Issue 仮案踏襲）。既定値はそれぞれ:
- `PR_REVIEWER_CODEX_CMD="codex review --base {BASE}"`
- `PR_REVIEWER_ANTIGRAVITY_CMD="antigravity review --base {BASE}"`

**プレースホルダ**: `{BASE}` / `{HEAD}` / `{PR}` の 3 種をサポート。`{BASE}` は AC 4.3 で明示要求あり、`{HEAD}` / `{PR}` は将来の柔軟性確保のため初期から導入（運用上ほとんどの外部ツールは PR 番号で diff 取得できるため有用）。

**根拠**:
- Issue 仮案踏襲で既存運用に近づける
- `{HEAD}` / `{PR}` の追加 placeholder は YAGNI に抵触するが、後から追加すると既存テンプレートの再評価が必要になるため初期投入のコストが小さい
- 既定値の `<tool> review` 形式は両ツールの一般的な慣習（`codex review` / `antigravity review` がドキュメントで主要 entry point）に沿う想定。実機検証は本 PR 後のフォローで実施（後述「確認事項」）

### Decision 3: 認証状況確認コマンドの仕様（Open Question 3 への回答）

**採用**: env で override 可能なコマンド文字列 `PR_REVIEWER_<TOOL>_AUTH_CMD` を導入。既定値:
- `PR_REVIEWER_CODEX_AUTH_CMD="codex auth status"`
- `PR_REVIEWER_ANTIGRAVITY_AUTH_CMD="antigravity auth status"`

**仕様**:
- 空文字列なら認証チェックを **skip**（運用者が「認証チェック不要」と明示できる安全弁）
- 非空なら当該コマンドを実行し、終了コード 0 で認証 OK 判定。stdout/stderr は破棄
- 結果出力（OAuth token などを含み得る）は PR コメントに残さない

**根拠**:
- Issue 本文「判断を委ねたい点」に該当。既定値の妥当性は実機で検証が必要なため、env override で運用者が修正できる経路を必ず残す
- 認証チェック skip オプションは、運用者が事前に環境変数 / OS Keychain で認証を保証している場合に余計な subprocess 呼び出しを削減

### Decision 4: `PR_REVIEWER_ITERATION_PATTERN` の既定値（Open Question 4 への回答）

**採用**: `(?i)(needs[- ]iteration|要修正|fix\s+required|please\s+fix|修正してください)` を grep -E 互換の ERE として既定。

**根拠**:
- AC 5.1 はキーワードを env 指定可能とするが、既定値の妥当性は本 PR で決める必要がある
- 日本語・英語両方で「修正要求」を表す典型語を含める（idd-claude 自体が日英混在運用）
- `needs-iteration` の単語そのものはノイズ（運用者が PR コメントで触れた瞬間に発火）になりやすいが、既存 `pr-iteration.sh` が `needs-iteration` ラベルを起動条件とするため、AI レビュー結果に `needs-iteration` 単語が出現したら付与する挙動は **意図と整合**する
...
1. **既定実行コマンドの妥当性**: `codex review --base {BASE}` / `antigravity review --base {BASE}` が実機の両ツール CLI と整合するか（製品ドキュメント / 実機検証）。整合しなければ Decision 2 の既定値を修正
2. **認証チェックコマンドの妥当性**: `codex auth status` / `antigravity auth status` が実機で存在するか。存在しない場合は別コマンド or `pr_check_tool_authenticated` の既定挙動を「skip」に倒すかを判断（Decision 3）
3. **既定キーワードパターンの妥当性**: 日本語キーワード（`要修正` / `修正してください`）の混在は意図したものか、英語のみに絞るべきか（Decision 4）
4. **`{HEAD}` / `{PR}` placeholder 追加の是非**: AC 4.3 は `{BASE}` のみ明示要求。追加 placeholder を入れない案も可（YAGNI 観点）。設計 PR レビュー時に最終確認（Decision 2）
5. **`eval` 採用の Security Risk 受容**: 運用者が env で実行コマンドを定義できる設計は、`PROMOTE_CRON` / `AUTO_REBASE_TEMPLATE` 等の既存 env が **コマンドではなくテンプレート PATH** を受け取る慣習とはやや異なる。`eval` 採用を許容するか、`bash -c "$resolved_cmd"` で独立 subshell に閉じ込めるかは運用ポリシー次第（本書は防御的 metacharacter scan で軽減）
