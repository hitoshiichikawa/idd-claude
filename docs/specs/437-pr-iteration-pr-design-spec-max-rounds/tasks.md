...
- [x] 2. Adjudicator / Reviewer プロンプトの out-of-scope 判定指針を明文化
- [x] 2.1 adjudicator-prompt.tmpl に第 3 判定 out-of-scope を追記
...
- [x] 2.2 iteration-prompt.tmpl / iteration-prompt-design.tmpl に Developer 構造化マーカー出力指示を追記
...
- [x] 3. adjudicator.sh の 3 値分類対応（schema 検証 / label / comment）
...
- [x] 4. pi_general_filter_oos による out-of-scope コメント除外 (P)
...
- [x] 5. out-of-scope ルーティング共通ヘルパ pi_route_out_of_scope_escalate
...
- [x] 6. Developer 構造化マーカー検出 pi_detect_developer_oos_marker (P)
...
- [x] 7. 内容ベース no-progress 判定（fingerprint / streak / marker 永続化）(P)
...
- [x] 8. pi_run_iteration への配線（Developer marker 打ち切り / 内容ベース早期打ち切り）
...
- [x] 9. impl Reviewer プロンプト明文化 + root↔repo-template 同期
...
- [x] 10. Developer プロンプト明文化 + README 反映 + 全体検証
